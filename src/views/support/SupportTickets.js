...
const SupportTickets = () => {
  ...
  const [AllContact, Filterd] = useState();
  ...
  useEffect(() => {
    if (text === '') {
      ...
      const filteredObject = contact?.filter((item) => {
        return item?.fullName?.toLowerCase()?.includes(text.toLowerCase()) || item?.emailAddress?.toLowerCase()?.includes(text.toLowerCase());
      });
      Filterd([...filteredObject]);
    }
  }, [text, contact]);
  const filter = (e) => {
  ...
  return (
    <>
      ...
      <Col>
        ...
        <Row className="mb-3">
          <Col md="5" lg="3" xxl="4" className="mb-1">
            {/* Search Start */}
            <div className="d-inline-block float-md-start me-1 mb-1 search-input-container w-100 shadow bg-foreground">
              <Form.Control onChange={filter} type="text" placeholder="Search" />
